...
cmake --build engine/build-ptxas -j"$(nproc)" 2>&1 \
  | grep -E "ptxas info|Function properties|registers|spill|stack frame|Compiling entry" \
  | tee -a "$BUILDLOG"
...
for bad in "--group 3" "--group 0" "--group 33" "--pair-order sideways" \
  ...
  if ./engine/build/pearson_engine_nccl data/fixtures/item_tiny $bad >/dev/null 2>&1; then
...
if ./engine/build/pearson_engine_nccl data/fixtures/item_tiny --mode sync \
     --finalize-stream separate >/dev/null 2>&1; then
...
if command -v compute-sanitizer >/dev/null; then
  ...
  for g in 1 8 32; do
    out=$(compute-sanitizer --tool memcheck --error-exitcode 9 \
          ...
          --pair-order bylen 2>&1 | tail -3) && r=clean || { r=ERRORS; fail=1; }
    echo "memcheck nccl2 g=$g: $r" | tee -a "$LADDER"
    [ "$r" = clean ] || echo "$out" | tee -a "$LADDER"
  done
else
