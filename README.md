...
(seed 20260906), at the clean frozen commit `cfb8993` with the binaries' real
defaults -- which since the warp-packing work means `--group 4
--pair-order source`. Times are **steady-state**: `device_total = stats +
...
| --- | --- | --- |
| Serial C++ (oracle) | 1323.427 ms | 1.00x |
| OpenMP 16t dynamic | 86.039 ms | 15.38x |
| MPI 16 ranks | 278.317 ms | 4.76x |
| CUDA 1 GPU | 4.188 ms | 315.97x |
| **NCCL sync `packed`, 2 GPU** | **2.657 ms** | **498.09x** |
| NCCL async `packed`, 2 GPU | 3.482 ms | 380.02x |

**Replicated.** The same matrix ran on a second, independent pod
(`bench_20260906_002051.json`, different physical GPUs): serial within 0.08%,
`CUDA 1 GPU` identical to three decimals at 4.188 ms, `NCCL sync packed 2 GPU`
2.672 vs 2.657 ms. Every row agrees within 1.8%.
...
Log scale, because the span is 1323.4 ms to 2.7 ms. Whiskers are the
...
Measured on a **clean checkout of `cfb8993`** — no tracked file modified,
