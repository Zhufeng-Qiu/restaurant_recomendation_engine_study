...
(seed 20260906), at the frozen commit `1abc477` with the binaries' real
...
| --- | --- | --- |
| Serial C++ (oracle) | 1324.501 ms | 1.00x |
| OpenMP 16t dynamic | 87.624 ms | 15.12x |
| MPI 16 ranks | 283.125 ms | 4.68x |
| CUDA 1 GPU | 4.188 ms | 316.22x |
| **NCCL sync `packed`, 2 GPU** | **2.672 ms** | **495.60x** |
| NCCL async `packed`, 2 GPU | 3.461 ms | 382.69x |
...
These were measured on `1abc477`, which is the current tree apart from
harness-only commits after it (the bench self-test and this table generator).
Nothing in `engine/src` changed after the freeze.

![CPU vs GPU backend latency on item_full](results/figures/gpu_comparison.png)

Log scale, because the span is 1324.5 ms to 2.7 ms. Whiskers are the
...
- **On NVLink, compression is now worth having and async still is not.**
  Compressing the collective 3x buys **9.38% [-10.10, -8.66]** (30 balanced
  crossover blocks) against 4.3% before, because a 36% faster kernel leaves
  communication a larger share to attack — it is 9.1% of the iteration at
