...
>
> **It is not the GPUs settling.** A pure sleep at the same point buys only
> −2.00% at 500 ms against the diagnostics' −17.79%, and NVML sampled as the
> timed region opens reports the same SM clock (1155 MHz), memory clock and
> zero throttle reasons in *every* arm. So idle time explains about a ninth of
> it. The rest tracks the host work, and the split between the backends says
> where it lands: NCCL times its stats phase with host wall clock around launch
> and synchronize, the single-GPU backend uses CUDA events — and only the
> wall-clock one moved. A meaningful part of NCCL's reported `t_stats` is host
> overhead, not kernel time. See the experiment document for what that does and
> does not establish.
