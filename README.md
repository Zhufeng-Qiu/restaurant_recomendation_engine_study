...
(seed 20260906), at commit `174bb1c` — engine-identical to the frozen
`f4c5a61` — with the binaries' real defaults, which since the warp-packing work
means `--group 4 --pair-order source`. Times are **steady-state**: `device_total = stats +
...
**Replicated across three independent hosts**, same engine sources, distinct
GPU UUIDs. This table is the median host; the other two are
`bench_20260906_034430.json` and `bench_20260906_044505.json`:

| | host A | host B (above) | host C | spread |
| --- | --- | --- | --- | --- |
| Serial | 1262.053 | 1253.179 | 1250.637 | 0.9% |
...
The headline speedup therefore carries about 5% of host-to-host uncertainty,
which is larger than any single run's IQR and is the honest error bar on it.

> ⚠ **These numbers are ~16% slower than an intermediate version of this table,
> and the intermediate version was measured with something running that no
> longer runs.** While the lane-plan diagnostics were computed unconditionally
> they did ~490 ms of host work between `ncclCommInitAll` and the timed region.
> Making them opt-in removed it and the two-GPU sync time rose from 2.66 to
> 3.17 ms — reproducibly: putting the work back with `--plan-metrics on`
> recovers it, **−15.37% [−16.17, −14.57]** over 30 paired blocks, entirely in
> `t_stats`, with AllReduce unmoved and the one-GPU control null.
>
> **A pure wait does not reproduce it.** A sleep at the same point moves the
> median by −2.00% at 500 ms against the diagnostics' −17.79%, and the retained
> NVML samples show no clock or throttle difference between arms — though those
> are one sample per arm taken before setup, not at the stats phase, so they
> narrow the possibilities rather than excluding the GPUs. The suggestive part
> is which backend moved: NCCL times its stats phase with host wall clock
> around launch and synchronize, the single-GPU backend uses CUDA events, and
> only the wall-clock one changed. That points at host overhead inside NCCL's
> `t_stats` rather than kernel time, but the mechanism is **open** — see the
> experiment document.

Generated by `engine/bench/headline_table.py` from the run's own JSON rather
than transcribed, because every earlier version of this table drifted from the
run it claimed to describe.

Every version before 2026-09-05 mixed hardware — MPI from a laptop, GPU rows
from pods — so its speedups were computed against two different serial
baselines. This one is not. The CPU rows are within 1.2% of the previous
same-machine run (`serial` 1318.88 → 1324.50 ms); the GPU rows moved because
the lane mapping changed.

These were measured on `cfb8993`, the current tree apart from harness-only
commits after it. Nothing in `engine/src` changed after the freeze.

![CPU vs GPU backend latency on item_full](results/figures/gpu_comparison.png)

Log scale, because the span is 1253.2 ms to 3.2 ms. Whiskers are the
interquartile range over 30 trials: note that `NCCL async 2 GPU` has a
visibly wider one than every other bar -- 11.2% here against 1.1-3.3% for
every other row. That spread survives randomisation and reproduces across
pods -- see the measurement audit -- so the picture is the finding, not an
artefact of one session.

Key takeaways:

- **298-395x over one CPU core** on the same machine, ~22-30x over the best
  16-thread OpenMP configuration on that machine.
- **A second GPU now adds 1.46x**, up from 1.21x before warp packing. Only the
  rating dimension is split, so every GPU still touches every pair and the
  per-pair work does not halve — but splitting the dimensions also shortens
  every row, which is where a four-lane group helps most. The two-GPU
  configuration gained 32.8% from the new default against the one-GPU
  configuration's 9.9%.
- **On NVLink, compression is now worth having and async still is not.**
  Compressing the collective 3x buys **8.78% [-9.40, -8.16]** (30 balanced
  crossover blocks; a second independent session gives −9.38% [−10.10, −8.66])
  against 4.3% before, because a 36% faster kernel leaves communication a
  larger share to attack — it is 9.1% of the iteration at
  `packed` and 15.8% at `f64`. Async overlap now *costs* 29.5%, up from 9.3%:
  sync absorbed the kernel speedup and the pipeline did not.
...
costs ~100 ms of sorting against a ~3 ms kernel, so it is opt-in rather than
default: a resident engine wants it, a single CLI invocation does not. Payback
is **about 57 queries on one GPU and about 257 on two** — the plan costs the
same either way while the per-query saving is smaller in absolute terms, so the
single-GPU figure must not be quoted for a two-GPU deployment.
