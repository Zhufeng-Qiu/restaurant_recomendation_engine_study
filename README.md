...
> and the intermediate version was the wrong one.** While the lane-plan
> diagnostics were computed unconditionally, they inserted ~490 ms of host-side
> work between `ncclCommInitAll` and the timed region — an accidental settling
> period. Making them opt-in removed it and the two-GPU sync time rose from
> 2.66 to 3.17 ms. That this is the cause is measured, not inferred: putting
> the pause back with `--plan-metrics on` recovers it exactly, **−15.37%
> [−16.17, −14.57]** over 30 paired blocks, entirely in `t_stats` (−16.92%)
> with AllReduce unmoved (+0.37%, CI spanning zero), while the same test on one
> GPU is null (+1.36% [−1.64, +4.44]). Two GPUs draw twice the power in one
> chassis, so the duty cycle matters there and not on one.
>
> The pre-packing baseline had no plan at all and so no pause, which makes it
> comparable to these: warp packing takes NCCL sync 2 GPU from 3.977 to 3.170
> ms, **−20.3%**, and the headline speedup from 331.6x to 395.3x. The −33% that
> the intermediate runs suggested was that accidental pause, not the kernel.
